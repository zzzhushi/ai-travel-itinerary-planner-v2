...
rotation is deferred past M0 (see the milestone close-out).
"""
...
def configure_logging(stream: TextIO | None = None) -> None:
    if stream is None:
        ...
        log_file = settings.log_file.open("a", encoding="utf-8")
        sink: TextIO = cast("TextIO", _Tee(sys.stdout, log_file))
    else:
        sink = stream
    structlog.configure(
        processors=[
