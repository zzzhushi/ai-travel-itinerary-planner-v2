...
Not a milestone retro (M3's still runs post-merge and may amend). Shirley's review: the pipeline "implements correct-working code" but misses senior-engineer behavior — no design step, no refactor discipline, gold-plating, giant batches. Changes, each evidence-backed:
...
- **No refactor leg / duplication at write time** (`_hhmm`×2, `TravelMinutes`×4 deferred to nowhere) → ponytail-style **decision ladder** (guardrail 8) before any new code; **prep refactors** in-scope as own `refactor:` commits; guardrail 7 rewritten — clean what you modify, leave adjacent code alone; **ceiling comments** (guardrail 9, scoped).
- **Deferred findings evaporate in PR comments** → `docs/debt.md` ledger (guardrail 11), 3rd-sighting promotion rule; `/milestones` Gate 2 reads it; `/ship` routes findings there.
