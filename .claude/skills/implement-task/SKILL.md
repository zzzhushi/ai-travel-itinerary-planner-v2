...
2. **Failing test first.** If the task's behavior isn't already pinned by a committed failing test, write one now and watch it fail for the right reason. If you cannot write the failing test, the task is under-specified — escalate, don't improvise. (UI/visual tasks: write the manual validation step instead.)
...
4. **Verify:** `uv run ruff check && uv run mypy && uv run pytest -q`. Fix until green. Same error surviving 3 distinct fix attempts → stop, write a diagnosis (what you tried, current hypothesis), escalate.
...
## `--local` (experimental local-LLM routing)

Only for tasks rated **mechanical**, and only when the user passed `--local`. Process in `references/local-llm.md`; mechanics in `scripts/local_task.py`. Non-mechanical task + `--local` → tell the user it doesn't qualify and proceed on the cloud model. Every local run is followed by a top-model review of the diff before commit, and logged for the economics experiment.
