...
def extract_files(output: str) -> dict[str, str]:
    """Parse ===FILE: path=== blocks. Tolerant of the two ways local models drift
    from the protocol: wrapping the body in markdown fences, and ending the block
    with ``` instead of ===END===. The block ends at the first of ===END===, the
    next ===FILE:, or end-of-output."""
    files: dict[str, str] = {}
